...
async def test_get_weather_newtaipei(monkeypatch):
    ...
    async def mock_fetch_weather_data(url: str):
        ...
        elif "mock_forecast_url" in url:
            return {
                "properties": {
                    "periods": [
                        {"temperature": 28, "shortForecast": "Partly Cloudy"}
                    ]
                }
            }
...
async def test_get_forecast_newtaipei(monkeypatch):
    async def mock_fetch_weather_data(url: str):
        if "points" in url:
            ...
            return {"properties": {"periods": [{"temperature": 30, "shortForecast": "Sunny"}]}}
        raise Exception("Unexpected URL")
    monkeypatch.setattr("main.fetch_weather_data", mock_fetch_weather_data)
    lat = 25.0124
    lon = 121.4657
...
async def test_get_alerts_dc(monkeypatch):
    async def mock_fetch_weather_data(url: str):
        if "alerts/active" in url:
            return {
                "features": [
                    {"properties": {"event": "Flood Warning", "headline": "Flooding expected", "description": "Heavy rain in DC."}},
                    {"properties": {"event": "Heat Advisory", "headline": "High temperatures", "description": "Stay hydrated."}}
                ]
            }
        raise Exception("Unexpected URL")
    monkeypatch.setattr("main.fetch_weather_data", mock_fetch_weather_data)
